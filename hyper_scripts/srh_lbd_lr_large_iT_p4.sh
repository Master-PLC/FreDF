...
for mdl in "iTransformer"; do
    ...
    for dst in "Traffic"; do
        ...
        #     for lr in 0.0002; do
        #         job_name=${exp_name}_dst_${dst}_lbd_$(printf "%g" "$lbd")_lr_$(printf "%g" "$lr")_mdl_${mdl}
        #         if [[ $lbd == "0.0" ]] || [[ $lbd == "0.8" ]] || [[ $lbd == "1.0" ]]; then
        #             bash ./scripts/ICML2024/ltf_srh_lambda_lr_large_iT_supp/${dst}_script/${mdl}.sh ${lbd} ${lr} "./${exp_name}/${job_name}" ${gpu} 720
        #         else
        #             bash ./scripts/ICML2024/ltf_srh_lambda_lr_large_iT_supp/${dst}_script/${mdl}.sh ${lbd} ${lr} "./${exp_name}/${job_name}" ${gpu} 336 720
        #         fi
        #     done
        # done
        ...
        #     for lr in 0.0005; do
        #         job_name=${exp_name}_dst_${dst}_lbd_$(printf "%g" "$lbd")_lr_$(printf "%g" "$lr")_mdl_${mdl}
        #         if [[ $lbd == "0.0" ]] || [[ $lbd == "0.2" ]] || [[ $lbd == "0.3" ]] || [[ $lbd == "0.5" ]] || [[ $lbd == "0.8" ]] || [[ $lbd == "1.0" ]]; then
        #             bash ./scripts/ICML2024/ltf_srh_lambda_lr_large_iT_supp/${dst}_script/${mdl}.sh ${lbd} ${lr} "./${exp_name}/${job_name}" ${gpu} 720
        #         else
        #             bash ./scripts/ICML2024/ltf_srh_lambda_lr_large_iT_supp/${dst}_script/${mdl}.sh ${lbd} ${lr} "./${exp_name}/${job_name}" ${gpu} 336 720
        #         fi
        ...
        for lbd in 1.0; do
            for lr in 0.00002; do
                ...
                if [[ $lbd == "0.5" ]] || [[ $lbd == "0.6" ]] || [[ $lbd == "0.7" ]] || [[ $lbd == "1.0" ]]; then
                    command_args+=(720)
                elif [[ $lbd == "0.1" ]] || [[ $lbd == "0.9" ]]; then
                    command_args+=(336)
                else
